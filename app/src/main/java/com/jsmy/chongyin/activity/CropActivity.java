...
public class CropActivity extends BaseActivity {
    ...
    public void saveFile(Bitmap bm) throws IOException {
        MyLog.showLog("CCC", "-- " + cropWidth + " --");
        File file;
        if (cropWidth == 250) {
            file = new File(ServiceCode.BASE_PATH, ServiceCode.BASE_IMG_TX);
        } else {
            file = new File(ServiceCode.BASE_PATH, ServiceCode.BASE_IMG_CROP);
        }
        if (file.exists()) {
            file.delete();
        }
...
//            EventBus.getDefault().post(new DowloadEvent("upbackgruop", "upbackgruop"));
    ...
    public Bitmap getimage(String srcPath) throws IOException {
        ...
        float hh = cropHeight;// 这里设置高度为800f
        float ww = cropWidth;// 这里设置宽度为480f
